...
struct amyplan_escaped_string amyplanyy_escape_string(char *orig, char quote)
{
  ...
  while (orig[i] != quote)
  {
    ...
    else if (orig[i+1] == 'u')
    {
      ...
      if (unicode <= 0x7F)
      {
        buf[j++] = abce_uchar_to_char((uint8_t)unicode);
        i += 6;
        continue;
      }
      if (unicode <= 0x7FF)
      {
        buf[j++] = abce_uchar_to_char((uint8_t)(0xc0|(unicode>>6)));
        buf[j++] = abce_uchar_to_char((uint8_t)(0x80|(unicode&0x3f)));
        i += 6;
        continue;
      }
      if (unicode <= 0xFFFF)
      {
        buf[j++] = abce_uchar_to_char((uint8_t)(0xe0|(unicode>>12)));
        buf[j++] = abce_uchar_to_char((uint8_t)(0x80|((unicode>>6)&0x3f)));
        buf[j++] = abce_uchar_to_char((uint8_t)(0x80|(unicode&0x3f)));
        i += 6;
        continue;
      }
